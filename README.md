...
This repository tracks [HKUDS/nanobot](https://github.com/HKUDS/nanobot) and publishes the fork as `nanobot-ai-tng`.

Fork-only additions currently include:

- ESPHome and Alexa voice-channel experiments.
- Deferred MCP tool schema loading via `resolve_tools`.
- Subagent improvements: configurable `subagentMaxIterations` and optional tool filtering.
- Extra OpenAI-compatible provider entries such as Parasail, Fireworks AI, and Together AI.
- Gateway webhook support and Telegram interactive/location helpers.

Voice setup details live in [VOICE_CONTROL.md](./VOICE_CONTROL.md). Upstream documentation below is kept close to upstream to reduce merge drift.
...
```bash
python -m pip install nanobot-ai-tng
```

If pip reports `externally-managed-environment` on macOS or Linux, use the one-command installer, `uv tool install nanobot-ai-tng`, `pipx install nanobot-ai-tng`, or install inside a virtual environment.
