...
def test_subagent_default_max_iterations_matches_agent_defaults(tmp_path):
    ...
    assert mgr.max_iterations == 15


def test_agent_loop_passes_subagent_max_iterations_to_subagents(tmp_path):
    """AgentLoop's configured subagent limit should be shared with spawned subagents."""
...
async def test_agent_loop_syncs_updated_max_iterations_before_run(tmp_path):
    ...
    # This fork decouples subagent limits from the main loop's max_iterations
    # (see subagent_max_iterations); _sync_subagent_runtime_limits mirrors that
    # value, so drive it here to exercise the pre-run sync.
    loop.subagent_max_iterations = 55
