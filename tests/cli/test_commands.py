...
# NOTE: Upstream's gateway tests for the standalone asyncio.start_server
# health server and its shutdown/MCP-cleanup ownership were removed in this
# fork. The fork serves /health from the WebhookServer (started inside the
# gateway run loop) instead of a separate health-server task, which was
# removed to avoid a port conflict (see commit 'remove redundant health
# server task'). Those upstream tests assume the standalone-server
# architecture and do not apply here.
