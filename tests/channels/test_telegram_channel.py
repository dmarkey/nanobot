...
async def test_on_message_location_content() -> None:
    """Location messages are published to bus as interactive location responses."""
    bus = MessageBus()
    channel = TelegramChannel(
        TelegramConfig(enabled=True, token="123:abc", allow_from=["*"], group_policy="open"),
        bus,
    ...
    assert "48.8566" in published[0].content
    assert "2.3522" in published[0].content
    assert published[0].metadata.get("_location_response") is True


@pytest.mark.asyncio
async def test_on_message_location_with_text() -> None:
    """Location messages with text still route as interactive location responses (text ignored)."""
    bus = MessageBus()
    channel = TelegramChannel(
        TelegramConfig(enabled=True, token="123:abc", allow_from=["*"], group_policy="open"),
        bus,
    ...
    assert len(published) == 1
    assert "51.5074" in published[0].content
    assert "-0.1278" in published[0].content
