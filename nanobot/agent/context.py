...
class ContextBuilder:
    ...
    @staticmethod
    def build_session_runtime_block(
        channel: str | None,
        chat_id: str | None,
        timezone: str | None = None,
        sender_id: str | None = None,
    ) -> RuntimeContextBlock | None:
        """Base runtime metadata plus voice-channel guidance for the current turn.

        Registered as a runtime context provider so the block is appended to the
        current user message under the shared runtime-context markers, alongside
        provider-owned blocks (goal, cli_apps, mcp).
        """
        ...
        if not content:
            return None
        return RuntimeContextBlock(source="session", content=content)
