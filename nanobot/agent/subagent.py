...
class SubagentManager:
    ...
    async def spawn(
        ...
        bg_task = asyncio.create_task(
            self._run_subagent(task_id, task, display_label, origin, status, tool_filter=tool_filter)
        )
        ...
        try:
            ...
            result = await self.runner.run(AgentRunSpec(
                ...
                max_iterations_message=(
                    "You have used all your available tool calls. Summarise:\n"
                    "1. What you accomplished\n"
                    "2. What remains to be done\n"
                    "3. Exact parameters to pass if spawning a new subagent to finish"
                ),
                error_message=None,
            ...
            if result.stop_reason == "tool_error":
                ...
                announce_status = "error" if result.stop_reason == "max_iterations" else "ok"
                final_result = result.final_content or "Task completed but no final response was generated."
                logger.info("Subagent [{}] completed ({})", task_id, result.stop_reason)
                await self._announce_result(task_id, label, task, final_result, origin, announce_status)
