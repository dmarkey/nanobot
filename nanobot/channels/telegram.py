...
class TelegramChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if self.config.inline_keyboards:
            self.logger.debug("Telegram inline keyboards enabled")
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        if msg.content and msg.content != "[empty message]":
            ...
            # Streaming edits the message live and can't attach a keyboard, so skip it when we have one.
            use_streaming = not is_progress and self.config.draft_streaming and reply_markup is None
            ...
            # Streaming needs live edits, so skip the rich path when streaming.
            if (
    ...
    async def _on_callback_query(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            self.logger.debug("Could not edit callback message: {}", e)
